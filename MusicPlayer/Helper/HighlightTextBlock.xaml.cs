using MusicPlayer.Helper.Shaders.Impl;
using System;
...
public partial class HighlightTextBlock : UserControl
{
    ...
    private static void OnTextPropertyChanged(DependencyObject d, DependencyPropertyChangedEventArgs e)
    {
        ...
        private void UpdateTextClip()
        {
            ...
            // 注意：不要设置 TextAlignment，否则会和 offsetX 冲突
            ...
            // 容器宽度：始终使用可用宽度，这样文本才能在正确位置对齐
            var containerWidth = constraintWidth > 0 ? constraintWidth : textWidth;

            // 根据TextAlignment计算起始点偏移，确保文本正确对齐
            // 只使用 offsetX，不使用 TextAlignment，避免双重偏移
            double offsetX = 0;
            switch (TextAlignment)
            {
                ...
                case TextAlignment.Right:
                    offsetX = containerWidth - textWidth;
                    break;
